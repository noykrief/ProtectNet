...
int packet_filter(struct __sk_buff *skb) {
    ...
    // Filter packets on port 456
    if (tcp.dest == htons(456) || tcp.source == htons(456)) {
        bpf_trace_printk("Packet detected on port 456: src_port=%d, dst_port=%d\\n", ntohs(tcp.source), ntohs(tcp.dest));
    }
...
print("Tracing packets on port 456... Press Ctrl+C to exit.")
try:
    b.trace_print()
